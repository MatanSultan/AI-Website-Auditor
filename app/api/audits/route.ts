...
import { checkSandboxDailyAuditQuota, clientIp, rateLimit, rateLimitHeaders } from "@/lib/rate-limit";
...
export function sandboxDailyLimitResponse(dailyQuota: Awaited<ReturnType<typeof checkSandboxDailyAuditQuota>>) {
  if (!dailyQuota) throw new Error("SANDBOX_DAILY_QUOTA_REQUIRED");
  return NextResponse.json({
    error: "SANDBOX_DAILY_LIMIT_REACHED",
    message: "מכסת הבדיקות היומית בסביבת הבדיקה הסתיימה. אפשר לנסות שוב מחר.",
  }, {
    status: 429,
    headers: { ...rateLimitHeaders(dailyQuota), "Retry-After": String(dailyQuota.retryAfterSeconds), "Cache-Control": "private, no-store" },
  });
}

export async function POST(request: NextRequest) {
  let environment;
  try { environment = getServerEnvironment(); } catch { return apiError("CONFIGURATION_ERROR", 503); }
  const ip = clientIp(request);
  const ipLimit = await rateLimit(`audit:${ip}`, environment.demoMode ? 100 : 5, 60 * 60_000);
  ...
  let target: URL;
  try { target = environment.demoMode ? normalizeUrl(parsed.data.url) : await assertPublicUrl(parsed.data.url); } catch { return apiError("UNSAFE_OR_INVALID_URL", 400); }
  const domainLimit = await rateLimit(`domain:${target.hostname}`, environment.demoMode ? 100 : 3, 60 * 60_000);
  ...
  const dailyQuota = await checkSandboxDailyAuditQuota(environment);
  if (dailyQuota && !dailyQuota.success) return sandboxDailyLimitResponse(dailyQuota);
  ...
  const response = NextResponse.json({ id, status: "QUEUED", demoMode: environment.demoMode }, { status: 202 });
