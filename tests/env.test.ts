...
it("accepts credential-free demo configuration", () => { vi.spyOn(console, "warn").mockImplementation(() => undefined); expect(validateEnvironment({ DEMO_MODE: "true", APP_ENV: "DEMO" })).toMatchObject({ demoMode: true, appMode: "DEMO", paypalConfigured: false }); });
...
it("accepts complete production configuration", () => expect(validateEnvironment(productionEnvironment())).toMatchObject({ demoMode: false, appMode: "SANDBOX", qstashConfigured: true, redisConfigured: true }));
