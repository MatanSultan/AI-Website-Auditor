...
Use a pooled Neon URL in `DATABASE_URL` for runtime traffic and a direct Neon URL in `DIRECT_URL` for Prisma migrations. `DEMO_MODE=false` requires `APP_ENV=SANDBOX|LIVE`, an HTTPS `APP_BASE_URL`, queue/Redis/cron credentials, Firecrawl, PageSpeed, one selected AI provider, and an exact 32-byte Base64 report key. Groq is the recommended controlled SANDBOX provider; OpenAI remains available as an alternative. PayPal values are optional only as a complete group; LIVE requires PayPal Live.
...
`npm run smoke:providers` makes real Firecrawl, PageSpeed and the selected AI-provider calls only for the explicitly approved `SMOKE_TEST_URL` and consumes quota. PayPal requires a separate manual Sandbox capture/webhook test. See [VERCEL_DEPLOYMENT.md](./VERCEL_DEPLOYMENT.md) and [PRODUCTION_CHECKLIST.md](./PRODUCTION_CHECKLIST.md).

MVP מלא לבדיקת אתרים בעברית: סריקה של עד 10 עמודים, PageSpeed במובייל, ניתוח מובנה באמצעות ספק AI נבחר, תוצאה ציבורית מוגבלת, דוח בתשלום ולכידת לידים.
...
- `AI_PROVIDER` — חובה מחוץ ל־Demo; ערך `groq` או `openai`.
- `GROQ_API_KEY` — סוד שרת בלבד כאשר `AI_PROVIDER=groq`.
- `GROQ_MODEL` — ברירת המחדל המומלצת ל־SANDBOX היא `openai/gpt-oss-120b`.
- `OPENAI_API_KEY`, `OPENAI_MODEL` — חלופה אופציונלית כאשר `AI_PROVIDER=openai`.
- `SANDBOX_DAILY_AUDIT_LIMIT` — מכסת audits יומית משותפת ב־SANDBOX; ברירת מחדל `10`.
...
ל־SANDBOX מבוקר מומלץ להגדיר `AI_PROVIDER=groq`, ‏`GROQ_API_KEY` בצד השרת ו־`GROQ_MODEL=openai/gpt-oss-120b`. ה־adapter משתמש ב־OpenAI-compatible API של Groq, מבקש JSON Schema ומאמת שוב כל תשובה ב־Zod. לחלופה אפשר להגדיר `AI_PROVIDER=openai` עם `OPENAI_API_KEY` ו־`OPENAI_MODEL`; אין צורך במפתח של הספק שאינו נבחר. תוכן האתר מוכנס כראיה לא־מהימנה, מקוצר ומופרד מהוראות המערכת.

429, שגיאות 5xx זמניות ותקלות רשת מקבלות לכל היותר שני ניסיונות חוזרים עם backoff תחום. timeout, תשובה פגומה או כשל ספק הופכים ל־`AI_ANALYSIS_FAILED`; אם קיימים ממצאים דטרמיניסטיים/PageSpeed, ה־Audit מסתיים ביושר כ־`PARTIAL`. מכסת Groq חינמית מתאימה לפיתוח ולתצוגת portfolio מבוקרת, ואינה קיבולת מסחרית מובטחת או בלתי מוגבלת.
...
Production configuration is validated at startup and has no memory-store fallback. Set `DEMO_MODE=false`, PostgreSQL, Firecrawl, PageSpeed and credentials for the selected AI provider. Generate the report key with `node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"`; never reuse example text. PayPal additionally requires `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `NEXT_PUBLIC_PAYPAL_CLIENT_ID`, and `PAYPAL_WEBHOOK_ID`.
